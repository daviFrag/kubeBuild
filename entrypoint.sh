...
helm upgrade production ./deploy --install --set-string --set image.repository=ghcr.io/${GITHUB_REPOSITORY}@sha256:${GITHUB_SHA} --namespace="${KUBE_NAMESPACE}" --set image.secret=${IMAGE_NAME}-${KUBE_NAMESPACE} --timeout 30m0s
